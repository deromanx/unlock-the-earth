...
def extract_landmarks(ep_num, prompt_template=GEMINI_PROMPT_TEMPLATE):
    ...
    print(f"  ✓ Ep.{ep_num}: {len(valid)} landmarks → {output_path.name}")
    return True
...
def main():
    ...
    todo = [e for e in eps if not already_done(e)]
    ...
    for i, ep_num in enumerate(todo):
        ...
        if extract_landmarks(ep_num, prompt_template):
            success += 1
        else:
            fail += 1
        if i < len(todo) - 1:
